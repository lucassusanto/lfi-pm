...
<form class="form-horizontal" method="POST" action="{{ url('asset/'.$asset_id.'/contract/commit_edit') }}">
    ...
    <div class="form-group">
        <label class="control-label col-sm-3" for="asset_contract">Note:</label>
        <div class="col-sm-6">
            <textarea class="form-control" id="asset_contract" name="contract" required>{{ $data->contract }}</textarea>
        </div>
    </div>
</form>
@endsection