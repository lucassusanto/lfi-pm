    ...
    <div id="contract_add" class="tab-pane fade">
        <div class="form-horizontal">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-3">
                    <input type="hidden" id="add_contract_contract" name="contract" value="{{ $contracts[0]->id }}">
...
function fetchContractOptions(tag) {
    $.get('{{ url('ajax/contract/getOptions') }}', function(data) {
        ...
        $('#edit_contract_contract_id').val('');
        $('#edit_contract_contract').empty();
        $('#edit_contract_status').empty();

        $('#edit_contract_contract_id').val(contract[0].id);

        for(var i = 0, len = contract.length; i < len; i++) {
            $('#edit_contract_contract').append($('<option>', {
                id: contract[i].id,
        ...
        for(var i = 0, len = status.length; i < len; i++) {
            $('#edit_contract_status').append($('<option>', {
                text: status[i]
            }));
        }
    });
